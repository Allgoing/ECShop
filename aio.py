...
def create_pool(loop, **kw):  # 这里的**kw是一个dict
    ...
    # wait_closed()
    __pool = yield from aiomysql.create_pool(host=kw.get('host', 'localhost'), port=kw.get('port', 3306),
                                             user=kw['user'], password=kw['password'], db=kw['db'], charset=kw.get('charset', 'utf8'),
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    @asyncio.coroutine
    # 显示方言错误是什么鬼。。。 def update(self):  # 修改数据库中已经存入的数据
